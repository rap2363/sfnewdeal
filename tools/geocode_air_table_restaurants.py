...
AIR_TABLE_URL = 'https://api.airtable.com/v0/appkknBvhJoouuyAl/Great%20Plates%20Restaurants'
...
def update_rows(rows_to_update):
    ...
    for i in range(0, len(rows_to_update), BATCH_UPDATE_SIZE):
        ...
        print(batch_to_update)
        resp = requests.patch(AIR_TABLE_URL, auth = utils.BearerAuth(AIR_TABLE_API_KEY), json = batch_to_update)
...
if __name__ == '__main__':
    all_rows = utils.get_all_rows_with_filter(AIR_TABLE_URL, AIR_TABLE_API_KEY, filter_string = "{Google Place ID} = ''")
    print(len(all_rows))
    rows_to_update = []
    for row in all_rows:
